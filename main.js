...
const a229037_antiprogression = depth => {
	const result = new Int32Array (depth)
	let progress = 0
	let maxVal = 1
	while (progress < depth) {
...
const A309890 = depth => {
	...
	while (progress < depth) {
		...
		let steps = 1
		while (steps * 2 < progress + 1) {
			const p1 = result [progress - steps]
			const p2 = result [progress - steps * 2]
...
const A309890_decreasing = depth => {
	const result = new Int32Array (depth)
	let progress = 0
	let maxVal = 1
	while (progress < depth) {
